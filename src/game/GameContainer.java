...
public class GameContainer extends JFrame {
    ...
    private final JButton menuButton = new JButton("Enter");

    private static final String[] shipNames = {"Carrier", "Battleship", "Submarine", "Destroyer", "Patrol Boat"};
    private static final String ships = "CBSDP";
    private static final String index = "ABCDEFGHIJ";
    ...
    public void init() throws IOException {
        ...
        //network
        connectionHandler.showIPDialog();
            ...
            while (lineRead != null) {
                for (int row = 0; row < board.length; row++) {
                    ...
                    for (int col = 0; pieces.hasMoreTokens(); col++) {
                        ...
                        if (temp == 'H' || temp == 'M') {
                            if (isCpu == true) {
                                enemyBoard.getButtons()[row][col].setIcon(new ImageIcon(IMAGE_PATH + temp + ".jpg"));
                                board[row][col + 1].setText("" + temp);
                            } else {
                                board[row][col + 1].setIcon(new ImageIcon(IMAGE_PATH + temp + ".jpg"));
                            }
                        } else {
                            if (isCpu == true) {
